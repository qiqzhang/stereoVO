...
namespace myslam
{
    const int ORBextractor::TH_HIGH = 100;
    const int ORBextractor::TH_LOW = 50;
    const int ORBextractor::HISTO_LENGTH = 30;
                                       ...
                                       const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
{
    ...
    for(size_t i=0;i<vToDistributeKeys.size();i++)
    {
        ...
        vpIniNodes[kp.pt.x/hX]->vKeys.push_back(kp);    //vector[1.2]=vector[2]
    }
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        for(int i=0; i<nRows; i++)
        {
            ...
            for(int j=0; j<nCols; j++)
            {
                ...
                FAST(mvImagePyramid[level].rowRange(iniY,maxY).colRange(iniX,maxX),        //提取的兴趣点的坐标不包含边界值，最后要加上
        ...
        for(int i=0; i<nkps ; i++)
        {
            ...
            keypoints[i].octave=level;                    //标记金字塔层级
            keypoints[i].size = scaledPatchSize; //相邻直径
        }
    }
